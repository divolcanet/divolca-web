  ...
  return (
    <footer className=" bg-black text-white">
        ...
        <div className=" flex flex-col items-center text-center md:items-start md:text-start gap-6">
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map(({ href, icon: Icon, label }) => (
              <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={label} className="text-volcanic-400 hover:text-magma-400 transition-colors">
      ...
      <div className=" bg-black py-6">
        <p className="text-center text-sm text-volcanic-500">© {new Date().getFullYear()} All Rights Reserved</p>
      </div>
    </footer>
  );
}
