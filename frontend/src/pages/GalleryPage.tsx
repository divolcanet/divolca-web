...
function PhotoCard({ photo, delay, onOpen }: { photo: Photo; delay: number; onOpen: () => void }) {
  ...
  return (
    <Reveal delay={delay}>
      <button
        ...
        <p className="absolute left-0 right-0 bottom-0 p-3 text-sm text-white translate-y-full group-hover:translate-y-0 transition-transform duration-300 line-clamp-2">{photo.caption}</p>
...
function Lightbox({ photo, onClose, closeLabel }: { photo: Photo; onClose: () => void; closeLabel: string }) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-page/95 flex items-center justify-center p-4" onClick={onClose}>
      <Button onClick={onClose} aria-label={closeLabel} size="icon" variant="outline" className="absolute top-4 right-4 bg-white">
        <X />
      </Button>
...
export default function GalleryPage() {
  ...
  return (
    <Container className=" bg-page flex flex-col items-center">
      <Reveal>
        <h1 className="font-fraunces text-4xl text-center font-bold text-title mb-4">{t.gallery.title[lang]}</h1>
        <p className="text-muted text-center max-w-3xl mb-10 mx-auto">{t.gallery.desc[lang]}</p>
      </Reveal>
      ...
      <div key={activeKey} className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
        {activeCategory.photos.map((photo, i) => (
          <PhotoCard key={photo.src} photo={photo} delay={i * 60} onOpen={() => setLightboxPhoto(photo)} />
        ))}
      </div>

      {lightboxPhoto && <Lightbox photo={lightboxPhoto} onClose={() => setLightboxPhoto(null)} closeLabel={t.gallery.close[lang]} />}
    </Container>
  );
}
