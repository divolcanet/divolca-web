  ...
  return (
    <div className=" bg-page">
      ...
      <section className="relative h-[60vh] min-h-105 flex items-end overflow-hidden">
        <img src={sileri} alt="Kawah Sileri, Dieng" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <Reveal className=" relative max-w-7xl mx-auto px-5  pb-12 w-full space-y-3 ">
          ...
          <h1 className="font-fraunces text-4xl sm:text-5xl font-bold text-title">{t.aboutDieng.heroTitle[lang]}</h1>
      ...
      <Container id="pendahuluan">
        <Reveal delay={300}>
          <h2 className="font-fraunces text-2xl font-semibold text-title mb-4">{t.aboutDieng.introTitle[lang]}</h2>
      ...
      <Container id="geolokasi">
        <Reveal>
          <h2 className="font-fraunces text-2xl font-semibold text-title mb-6">{t.aboutDieng.geoTitle[lang]}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
            {t.aboutDieng.geoStats.map((item) => (
              <div key={item.label.id} className="rounded-lg bg-card p-4 space-y-3">
                <p className="text-xs uppercase tracking-wide font-bold">{item.label[lang]}</p>
                <p className="text-primary-75 font-fraunces text-lg mt-1">{item.value}</p>
              </div>
            ))}
          </div>
          <div className="space-y-4 text-volcanic-300 leading-relaxed">
      ...
      <Container id="geologi">
        <Reveal>
          <h2 className="font-fraunces text-2xl font-semibold text-title mb-4">{t.aboutDieng.geologyTitle[lang]}</h2>
          <div className="space-y-4 text-volcanic-300 leading-relaxed">
      ...
      <Container id="sejarah-erupsi" className=" bg-secondary">
        <Reveal>
          <h2 className="font-fraunces text-2xl font-semibold text-primary-75 mb-8">{t.aboutDieng.eruptionTitle[lang]}</h2>
          <div className="relative border-l border-primary-50 pl-6 space-y-8">
            {aboutDieng.eruptionTimeline.map((item) => (
              <div key={item.year} className="relative text-black">
                ...
                <h3 className="font-fraunces font-bold text-lg mt-1">{item.title[lang]}</h3>
      ...
      <Container id="karakteristik">
        <Reveal>
          <h2 className="font-fraunces text-2xl font-semibold text-title mb-6">{t.aboutDieng.characteristicsTitle[lang]}</h2>
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {aboutDieng.characteristics.map((item) => (
              <div key={item.title.id} className="rounded-lg bg-card p-5 hover:shadow-md transition-all">
                <h3 className="text-primary-75 font-fraunces text-base font-semibold mb-2">{item.title[lang]}</h3>
                <p className="text-volcanic-300 text-sm leading-relaxed">{item.desc[lang]}</p>
              </div>
            ))}
          </div>
        </Reveal>
      ...
      <Container id="destinasi">
        <Reveal>
          <h2 className="font-fraunces text-2xl font-semibold text-title mb-2">{t.aboutDieng.destinationsTitle[lang]}</h2>
          <p className="text-volcanic-400 mb-6">{t.aboutDieng.destinationsDesc[lang]}</p>
            ...
            {destinations.map((dest) => (
              <div
                ...
                <div className="p-4 flex flex-col gap-2 flex-1">
                  <h3 className="text-primary-75 font-fraunces text-lg font-semibold">{dest.title}</h3>
                  <p className="text-volcanic-300 text-sm leading-relaxed">{dest.desc[lang]}</p>
      ...
      <Container id="reference">
        <details className="rounded-lg bg-card p-5">
          <summary className="cursor-pointer text-title font-fraunces text-base font-semibold">{t.aboutDieng.references[lang]}</summary>
          <ul className="mt-4 space-y-2 text-volcanic-400 text-sm leading-relaxed list-disc list-inside">
            ...
            <li>Dinas Kebudayaan, Pariwisata, dan Ekonomi Kreatif Provinsi Jawa Tengah. (2026). Bukit Sikunir, Kawah Sikidang, Talaga Warna.</li>
            <li>Giggenbach, W. F., et al. (1991). The Dieng 1979 Phreatic Eruption. Journal of Volcanology and Geothermal Research.</li>
            <li>Hamilton, W. (1979). Tectonics of the Indonesian Region.</li>
            <li>Hochstein, M. P., & Sudarman, S. (2008). History of Geothermal Exploration in Indonesia. Geothermics.</li>
            ...
            <li>Smithsonian Institution. (2024). Global Volcanism Program: Dieng Volcanic Complex.</li>
